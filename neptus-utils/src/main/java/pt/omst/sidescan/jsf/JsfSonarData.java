...
public class JsfSonarData {
    private static final Logger LOG = LoggerFactory.getLogger(JsfSonarData.class);
    ...
    */
    ...
    void parseHeader(ByteBuffer buf) {
        ...
        range = ((buf.getInt(116) / (float) Math.pow(10, 9)) * numberOfSamples * 1500) / 2.0f;
        range = Math.round(range);

        int msbStartFreq = (msb & 0x000F) << 16; // First 4 bits of msb shifted so only adding is needed to start Frequency
        frequency = ((buf.getShort(126) & 0xFFFF) + msbStartFreq) * 10.0f;
        ...
        if (numberOfSamples < 0)
            LOG.info("<###>number of samples below zero " + this);
