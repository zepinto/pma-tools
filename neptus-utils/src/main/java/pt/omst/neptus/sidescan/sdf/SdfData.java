...
public class SdfData {
    private static final Logger LOG = LoggerFactory.getLogger(SdfData.class);
    ...
    void parseData(ByteBuffer buf) {
        ...
        if (numSamples != buf.getInt(4)) {
            LOG.info("<###> " + SdfParser.class.getSimpleName() + " :: Sample size mismatch");
            return;
        }
     ...
     *
     */
    public void calculateTimeStamp(boolean useGpsTime) {
        ...
        if (!useGpsTime) {
            ...
            day = header.getDay();
            hour = header.getHour();
            minute = header.getMinute();
            fSeconds = header.getSecond() + header.gethSecond() / 100.0f;
        
        }
        else {
            year = header.getFixTimeYear();
            month = header.getFixTimeMonth();
            day = header.getFixTimeDay();
            hour = header.getFixTimeHour();
            minute = header.getFixTimeMinute();
            fSeconds = header.getFixTimeSecond();     
        }
